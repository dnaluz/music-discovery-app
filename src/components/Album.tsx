    ...
    <div className="w-full album">
      <div className="aspect-square">
        <div className="w-full h-full">
          {album.coverImage ? <a onClick={() => favorite(album) } className="relative w-full h-full block"><Image src={album.coverImage} alt={album.title} fill className="grayscale hover:grayscale-0 transition-all duration-300 hover:cursor-pointer" priority sizes="(max-width: 768px) 100vw, 100hw" /></a> : <div className="flex flex-col items-center w-full text-5xl font-semibold justify-center h-full relative">No Image</div>}
        </div>
      </div>
      <div className="mt-1">
        <div className="w-full flex flex-row items-center">
          <div className={`font-semibold ${isArtistView ? 'text-base' : 'text-xl'}`}>Album: <span className="album__title">{isArtistView ? artist : albumTitle}</span></div>
